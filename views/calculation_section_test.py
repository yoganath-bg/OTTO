...
with col1:
    ...
    if uploaded_file is not None:
        input_risk = pd.read_csv(uploaded_file)
...
def merge_lookup_value_loads(df, value_to_return, pricing_date, lookup_table):
    df = df.merge(
        lookup_table[['pricing_key', 'price_group', 'exec_rule', value_to_return]],
        how='left',  # Use 'left' join to keep all rows from the main DataFrame
        left_on=['pricing_key', 'price_group', 'exec_rule'],  # Columns from the main DataFrame
...
def merge_appl_level_lookup_value(df, value_to_return, pricing_date, lookup_table):
    # Separate the DataFrame into two parts: one for KAC and one for others
    df_kac = df[df['pricing_key'].isin(['KAI1', 'KAI2'])]
    df_others = df[~df['pricing_key'].isin(['KAI1', 'KAI2'])]

    # Merge for KAC products with appliance type
    df_kac = df_kac.merge(
        lookup_table[['pricing_key', 'price_group', 'exec_rule','appliance_type', value_to_return]],
        how='left',
        left_on=['pricing_key', 'price_group', 'exec_rule','appliance_type'],
        right_on=['pricing_key', 'price_group', 'exec_rule','appliance_type']
    )

    # Merge for other products without appliance type
    ...
    df = pd.concat([df_kac, df_others], ignore_index=True)
...
def merge_bundle_adj_lookup_value(df, value_to_return, pricing_date, lookup_table):
    # Separate the DataFrame into two parts: one for 'DNI1' and 'DNI2', and one for others
    df_pad = df[df['pricing_key'].isin(['DNI1', 'DNI2','2PDI','2PFI'])]
    df_others = df[~df['pricing_key'].isin(['DNI1', 'DNI2','2PDI','2PFI'])]

    # Merge for PAD products with 'geo_region'
    df_pad = df_pad.merge(
        lookup_table[['pricing_key', 'exec_rule', 'price_group', 'logical_bundle_key', 'geo_code', value_to_return]],
        how='left',
    ...
    # Merge for other products without 'product_tenure'
    df_others = df_others.merge(
        lookup_table[['pricing_key', 'exec_rule', 'price_group', 'logical_bundle_key',  value_to_return]],
        how='left',
    ...
    # Combine the two DataFrames back together
    df = pd.concat([df_pad, df_others], ignore_index=True)
...
def merge_cap_collar_lookup_value(df, value_to_return, pricing_date, lookup_table):
    # Separate the DataFrame into two parts: one for 'KAI1' and 'KAI2', and one for others
    df_kac = df[df['pricing_key'].isin(['KAI1', 'KAI2'])]
    df_others = df[~df['pricing_key'].isin(['KAI1', 'KAI2'])]

    # Merge for 'KAI1' and 'KAI2' with 'product_tenure'
    df_kac = df_kac.merge(
        lookup_table[['pricing_key', 'exec_rule', 'jobs_completed', 'product_tenure', value_to_return]],
        how='left',
    ...
    # Apply the value_to_return logic
...
def calculate_prices(input_risk):
    input_risk = merge_appl_level_lookup_value(input_risk, 'f_base_price', pricing_date, base_price_df)
    ...
    input_risk = merge_bundle_adj_lookup_value(input_risk, 'f_bundle_adj', pricing_date, bundle_tenure_df)
    ...
    input_risk = merge_lookup_value_loads(input_risk, 'f_Claims_load', pricing_date, claims_load_df)
    input_risk = merge_lookup_value(input_risk, 'f_Asv_price', pricing_date, asv_df)
    input_risk = merge_appl_level_lookup_value(input_risk, 'f_Exp_Claim_load', pricing_date, exp_claim_df)
    input_risk = merge_lookup_value_loads(input_risk, 'f_Exp_Asv_load', pricing_date, exp_asv_load)
    input_risk = merge_appl_level_lookup_value(input_risk, 'f_Flat_Load', pricing_date, flat_load_df)
    input_risk = merge_appl_level_lookup_value(input_risk, 'f_flat_margin', pricing_date, flat_bundle_df)
    input_risk = merge_appl_level_lookup_value(input_risk, 'f_Multi_bund_margin', pricing_date, multi_margin_df)
    ...
    input_risk['c_Risk_Premium'] = input_risk['f_base_price']*input_risk['f_bundle_adj']*input_risk['f_boiler_type']*input_risk['f_appl_make']*input_risk['f_geo_factor']*input_risk['f_boiler_age']*input_risk['f_rads']*input_risk['f_spl_rads']*input_risk['f_claims']*input_risk['f_int_geo_boiler']*input_risk['f_int_geo_rads']*input_risk['f_int_rads_claims']*input_risk['f_int_claims_boiler']*input_risk['f_Claims_load']
    ...
    input_risk['Pre_Cap_Premium'] = input_risk['Pre_Margin_Premium']+input_risk['Margin']+input_risk['f_flat_margin']
