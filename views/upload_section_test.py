...
def process_dataframe(df):
    ...
    def cleaned_boiler(s):
        ...
        if '>' in s:
            ...
            end_value = int(s[1:])
            return list(range(0, end_value + 1))
        elif re.match(r'\[\d+\.\.\d+\]', s):
            match = re.match(r'\[(\d+)\.\.(\d+)\]', s)
            if match:
                start, end = map(int, match.groups())
    ...
    # Applying cleaned age function to product tenure column
    if 'product_tenure' in df.columns and 'pricing_key' in df.columns:
        df.loc[df['pricing_key'].isin(['KAI1', 'KAI2']), 'product_tenure'] = df.loc[df['pricing_key'].isin(['KAI1', 'KAI2']), 'product_tenure'].apply(cleaned_boiler)
    ...
    if 'geo_code' in df.columns:
        df['geo_code'] = df['geo_code'].str.split(";")
        df = df.explode('geo_code', ignore_index=True)
    ...
    for i in ['exec_rule', 'price_group', 'logical_bundle_key']:
        if i in df.columns:
    ...
    if "exec_rule" in df.columns:
        df = df[(df['exec_rule'] == 14) | (df['exec_rule'] == 4)]
