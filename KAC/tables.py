...
col_dict = {'Services Pricing : Execution Rule':'exec_rule',
    ...
    'Manufacturer Code':'appl_make_factor',
    'Boiler Age' : 'boiler_age',
    'SPRC : RADS' : 'radiators',
    'No. Spec. Radiators' : 'spl_radiators',
    'No. of Jobs Complete':'jobs_completed',
    'Boiler Group' : 'boiler_group',
    'Product Tenure':'product_tenure',
    'Is Combi Boiler':'combi_boiler',
    'Logical Bundle Key' : 'logical_bundle_key'}
...
# # Applying cleaned age function to boiler_group column (check this with Delivery Team and see if this needed)
# if 'boiler_group' in df.columns:
#     df['boiler_group'] = df['boiler_group'].apply(cleaned_lists)
# # Explode the list into separate rows
#     df = df.explode('boiler_group').reset_index(drop=True)

# # Applying cleaned age function to product_tenure column (check this with Delivery Team and see if this needed)
# if 'product_tenure' in df.columns:
#     df['product_tenure'] = df['product_tenure'].apply(cleaned_lists)
# # Explode the list into separate rows
#     df = df.explode('product_tenure').reset_index(drop=True)
...
df = df[(df['pricing_date_start'] <= pricing_date) & (df['pricing_date_end'] >= pricing_date)]
